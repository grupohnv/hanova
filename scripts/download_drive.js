...
rclone('copy "gdrive:dados relatorios/cadastro-cliente-dimensão.xlsx" "' + DADOS + '/"');
rclone('copy "gdrive:dados relatorios/Produtos Hanova.xlsx" "' + DADOS + '/"');

// Bonificações (qualquer arquivo com "bonificad" no nome)
rclone('copy "gdrive:dados relatorios/" "' + DADOS + '/" --include "*onificad*"');

// Pasta de entrada (novos relatórios a processar)
rclone('copy "gdrive:dados relatorios/entrada/" "' + ENTRADA + '/"');

// Metas (todas as subpastas MM-YYYY)
rclone('copy "gdrive:comercial hanova/metas/" "' + METAS + '/" --max-depth 2');
